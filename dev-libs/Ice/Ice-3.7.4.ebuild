...
SRC_URI="https://github.com/zeroc-ice/ice/archive/v${PV}.tar.gz -> ${P}.tar.gz
	doc? ( http://download.zeroc.com/ice/$(ver_cut 1-2)/${PN}-3.7.1.pdf )"
LICENSE="GPL-2"
SLOT="0/37"
...
RDEPEND=">=dev-libs/expat-2.0.1
	...
	dev-cpp/libmcpp[static-libs]
	python? ( ${PYTHON_DEPS} )
	ruby? ( $(ruby_implementation_depend ruby24) )
	mono? ( dev-lang/mono )
	php? ( >=dev-lang/php-7.3 )
	!dev-python/IcePy
	!dev-ruby/IceRuby"
DEPEND="${RDEPEND}
...
PATCHES=(
	"${FILESDIR}/${PN}-3.7.2-php-zts.patch"
)
...
src_prepare() {
	...
	sed -i \
		...
		{php,python,ruby}/config/Make.rules || die "sed failed"
