...
src_prepare() {
	...
	epatch "${FILESDIR}/${PN}-1.27.0-staticdata.patch"

	# do not try to save exported html to python sitedir
	epatch "${FILESDIR}/${PN}-1.27.0-html-export-path.patch"
	...
	epatch "${FILESDIR}/${PN}-1.27.0-root-path.patch"

	# run with python2
	sed -ei "s:\(/usr/bin/env \)python$:\\1python2:" pyfa.py

	# remove useless file
	cenrm scripts/pyfa-setup.iss

	# do not copy file when copying the scripts directory
	mv scripts/iconIDs.yaml iconIDs.yaml
...
src_install() {
	pyfa_py_install() {
		...
		insinto "${packagedir}"
		doins -r eos gui imgs scripts service utils LICENSE
		[[ -e info.py ]] && doins info.py # only in zip releases
		doins "${BUILD_DIR}/configforced.py" "${BUILD_DIR}/config.py"
